...
public class TrafficAgent : MonoBehaviour {
	...
	void Update () {
		...
		if (timeSpan < 0) {
			...
			if (status == intmodel.phasesNumber - 1) 
				status = 0;
			else
				status += 1;

			intersection.OpenTurns = intmodel.phases[status];
		}
	...
	public struct IntersectionModel{
		...
		public IntersectionModel(Intersection intersection){
			...
			if (intersection.roads.Length == 4){
				...
				// TODO this is nasty. Better nesting strategy needed
				switch(outroads)
				{
				case 4:
					if(inroads == 4){
			...
			// TODO implement T intersection
			if (intersection.roads.Length == 3)
				;
